...
| No Logs | No IP addresses, no usernames, no message content stored in plaintext |
...
```
roomKey   = PBKDF2-SHA256(password, "roomkey:"  + roomId, 210,000 iterations)
joinProof = PBKDF2-SHA256(password, "joinproof:" + roomId, 210,000 iterations)
```

- Separate KDF domains prevent the joinProof hash from being used to derive roomKey
- Server-side PBKDF2 verification runs off the thread pool (`Task.Run`) — hub threads are never blocked
- Open rooms derive `roomKey` from the room ID only — users are warned via an amber UI notice
...
```
fingerprint = SHA256(roomId + nonce + ciphertext_base64)
```
Identical fingerprints within 120 seconds are rejected.
...
```csharp
// 256-bit entropy, OS-backed CSPRNG, generated fresh on every startup
adminToken = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32));
```

Minimum token length of 16 characters is enforced at startup; shorter tokens disable the endpoint entirely.
...
| Subresource Integrity | `integrity=sha256-...` on all script tags — browser-enforced |
| Security Headers | `X-Frame-Options: DENY`, nosniff, Referrer-Policy, COOP/CORP |
...
| No Third Parties | No analytics, CDN resources, WebRTC, or LocalStorage |
...
The **compiled binary** is free for personal and non-commercial use.  
The **source code** is available at [dev-offcode.com](https://www.dev-offcode.com/TorChatPage.html) ($29, personal license).  
Commercial use requires a separate license — contact via the website.
